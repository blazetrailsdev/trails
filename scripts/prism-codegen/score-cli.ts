...
import { scoreFile, type ScoreEntry } from "./score.js";
...
async function main() {
  ...
  console.log(
    `  ${"file".padEnd(34)} matched divergent missing  conformance (matched / present-in-both)`,
  );
  ...
  for (const f of TARGET_FILES) {
    ...
    const score = scoreFile(code, readFileSync(portPath, "utf8"), cleanDefs);
    ...
    console.log(
      `  ${short.padEnd(34)} ${String(score.matched).padStart(7)} ${String(score.divergent).padStart(9)} ` +
        `${String(score.missing).padStart(7)}  ${score.conformancePct.toFixed(1).padStart(6)}%`,
    );
  }

  console.log("  " + "-".repeat(92));
  const present = totMatched + totDivergent;
  console.log(
    `  ${"TOTAL".padEnd(34)} ${String(totMatched).padStart(7)} ${String(totDivergent).padStart(9)} ` +
      `${String(totMissing).padStart(7)}  ${(present ? (totMatched / present) * 100 : 0).toFixed(1).padStart(6)}%`,
  );
  console.log(
