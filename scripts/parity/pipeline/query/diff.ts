/**
 * Usage: tsx scripts/parity/pipeline/query/diff.ts --rails-dir <dir> --trails-dir <dir>
 *
 * Loads every *.json file in both dirs, validates each against
 * scripts/parity/pipeline/canonical/query.schema.json, diffs rails vs trails per
 * fixture, and prints per-fixture PASS/FAIL/KNOWN-GAP with a unified diff
 * on unexpected failures.
 *
 * Known-gap handling:
 *   - scripts/parity/pipeline/canonical/query-known-gaps.json lists fixtures where
...
const SCHEMA_PATH = "scripts/parity/pipeline/canonical/query.schema.json";
...
const KNOWN_GAPS_PATH =
  process.env.PARITY_KNOWN_GAPS_PATH || "scripts/parity/pipeline/canonical/query-known-gaps.json";
const FIXTURES_DIR = process.env.PARITY_FIXTURES_DIR || "scripts/parity/pipeline/fixtures";
// Pre-includes the v2 "ar-" AR-style prefix so widening the orchestrator's
// matcher (scripts/parity/pipeline/run.ts) doesn't silently un-classify new fixtures.
