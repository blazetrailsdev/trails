...
export interface MethodInfo {
  ...
  writer?: boolean;
  /**
   * TS-side only (RFC 0156): on a `get` accessor or a non-callable property,
   * whether its type admits `boolean` (a boolean member, `any` / `unknown`, or
   * a type parameter). Absent on methods. A Ruby predicate matched through its
   * bare candidate by a member that cannot hold a boolean is reported as a kind
   * mismatch — see compare.ts `predicateKindMismatch`.
   */
