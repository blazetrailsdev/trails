...
export interface MethodInfo {
   ...
   */
  writer?: boolean;
  /** TS side only (RFC 0156): a getter's or value property's type admits
   *  `boolean` (or `any` / `unknown` / a type parameter). Absent on methods. */
  admitsBoolean?: boolean;
  /** TS side only (RFC 0156): the signature returns `void` or `Promise<void>`. */
  returnsVoid?: boolean;
  /**
   * Ruby side only (RFC 0156): the kind of the body's final expression, the
   * value Ruby returns when nothing returned earlier — `call`, `new`,
   * `return` (`return <expr>`), `assign`, or `other`.
   */
  lastExpr?: "call" | "new" | "return" | "assign" | "other";
  /**
