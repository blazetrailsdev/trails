...
//
// Rails fires lifecycle callbacks from a handful of ported ActiveRecord
// methods via `_run_<event>_callbacks` (the private form generated by
// ActiveSupport::Callbacks.define_callbacks) or the public `run_callbacks(:event)`.
// The TS port mirrors this with `runCallbacks("<event>")` / `runAllCallbacks`.
// This manifest maps each such Rails method (converted to its TS candidate
// names) to the set of callback events its body invokes, so the
// `rails-callback-invocations` ESLint rule can require the ported method to
// keep firing them.
//
// Only the *own* (implicit-receiver) invocation is captured: an explicit
...
function callbackEventsInRuby(src: string): Map<string, Set<string>> {
  ...
  const lines = src.split("\n");
  const add = (method: string, event: string) => {
    let s = byMethod.get(method);
    if (!s) byMethod.set(method, (s = new Set()));
    s.add(event);
  };
  // Implicit-receiver `_run_<event>_callbacks` / `run_callbacks(:event)`.
  // The negative-lookbehind excludes explicit receivers (`.`), symbol-proc
  // (`&:`) and word chars so only self-directed invocations match.
  const underscore = /(?<![.&:\w])_run_([a-z_]+)_callbacks\b/g;
  const explicit = /(?<![.&:\w])run_callbacks\s*[( ]\s*:([a-z_]+)/g;
  for (let i = 0; i < lines.length; i++) {
    const code = stripComment(lines[i]);
    const events: string[] = [];
    for (const m of code.matchAll(underscore)) events.push(m[1]);
    for (const m of code.matchAll(explicit)) events.push(m[1]);
    if (events.length === 0) continue;
    const method = enclosingMethod(lines, i);
    if (!method) continue;
    for (const ev of events) add(method, ev);
  }
  return byMethod;
}
