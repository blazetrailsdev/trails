...
describe("SelectManagerTest", () => {
  ...
  it("takes the full outer join class", () => {
    const mgr = users.project(star).fullOuterJoin(posts, users.get("id").eq(posts.get("user_id")));
    expect(mgr.joinSources[0]).toBeInstanceOf(Nodes.FullOuterJoin);
    expect(mgr.toSql()).toContain("FULL OUTER JOIN");
  });

  it("takes the right outer join class", () => {
    const mgr = users.project(star).rightOuterJoin(posts, users.get("id").eq(posts.get("user_id")));
  ...
  it("takes an order with multiple columns", () => {
    const mgr = new SelectManager(users);
    mgr.project(
      users.get("id").over(mgr.window("w").order(users.get("id").asc(), users.get("name").desc())),
    );
    const sql = mgr.toSql();
