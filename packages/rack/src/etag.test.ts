...
describe("Rack::ETag", () => {
  ...
  it("not set cache-control if it is already set", async () => {
    const app = async () =>
      [201, { "content-type": "text/plain", "cache-control": "public" }, ["Hello, World!"]] as any;
    const response = await etag(app).call(request());
    expect(response[1]["cache-control"]).toBe("public");
  });
  ...
  it("not change etag if it is already set", async () => {
    const app = async () =>
      [200, { "content-type": "text/plain", etag: '"abc"' }, ["Hello, World!"]] as any;
    const response = await etag(app).call(request());
  ...
  it("not set etag if body is empty", async () => {
    ...
    const app = async () =>
      [200, { "content-type": "text/plain", "last-modified": now }, []] as any;
    const response = await etag(app).call(request());
  ...
  it("set handle empty body parts", async () => {
    const app = async () =>
      [200, { "content-type": "text/plain" }, ["Hello", "", ", World!"]] as any;
    const response = await etag(app).call(request());
    expect(response[1]["etag"]).toBe('W/"dffd6021bb2bd5b0af676290809ec3a5"');
  });
  ...
  it("not set etag if last-modified is set", async () => {
    const now = new Date().toUTCString();
    const app = async () =>
      [200, { "content-type": "text/plain", "last-modified": now }, ["Hello, World!"]] as any;
    const response = await etag(app).call(request());
    expect(response[1]["etag"]).toBeUndefined();
  });
  ...
  it("set etag even if no-cache is given", async () => {
    const app = async () =>
      [
        200,
        { "content-type": "text/plain", "cache-control": "no-cache, must-revalidate" },
        ["Hello, World!"],
      ] as any;
    const response = await etag(app).call(request());
    expect(response[1]["etag"]).toBe('W/"dffd6021bb2bd5b0af676290809ec3a5"');
