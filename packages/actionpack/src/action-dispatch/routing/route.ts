...
export class Route {
  ...
  get requestConstraints(): Record<string, unknown> {
    ...
    for (const k of Object.keys(this.constraints)) {
      if (!paths.has(k) && k in Request.prototype && !(k in Object.prototype))
        out[k] = this.constraints[k];
    }
    return out;
  }
  ...
  private matchVerb(requestMethod: string): boolean {
    const request = new Request({ REQUEST_METHOD: requestMethod });
    return this.requestMethodMatch.some((m) => m.call(request));
  }
