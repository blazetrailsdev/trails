...
describe("ActionController rescue_from", () => {
  it("rescues a known error", async () => {
    ...
    class C extends Base {
      async index() { throw new AppError("oops"); }
    }
    let rescued = false;
    C.rescueFrom(AppError, () => { rescued = true; });
  ...
  it("passes the error to the handler", async () => {
    class AppError extends Error { name = "AppError"; }
    class C extends Base {
      async index() { throw new AppError("specific message"); }
    }
    let message = "";
    C.rescueFrom(AppError, (err: Error) => { message = err.message; });
  ...
  it("does not rescue unregistered errors", async () => {
    class KnownError extends Error { name = "KnownError"; }
    class UnknownError extends Error { name = "UnknownError"; }
    class C extends Base {
      async index() { throw new UnknownError("nope"); }
    }
  ...
  it("rescues subclass errors", async () => {
    class BaseError extends Error { name = "BaseError"; }
    class SubError extends BaseError { name = "SubError"; }
    class C extends Base {
      async index() { throw new SubError("sub"); }
    }
    let rescued = false;
    C.rescueFrom(BaseError, () => { rescued = true; });
  ...
  it("child controller inherits rescue handlers", async () => {
    ...
    let handled = false;
    Parent.rescueFrom(AppError, () => { handled = true; });
  ...
  it("child can add its own rescue handlers", async () => {
    class ParentError extends Error { name = "ParentError"; }
    class ChildError extends Error { name = "ChildError"; }
    class Parent extends Base {
      async index() { throw new ChildError("child"); }
    }
    Parent.rescueFrom(ParentError, () => {});

    class Child extends Parent {}
    let childRescued = false;
    Child.rescueFrom(ChildError, () => { childRescued = true; });
  ...
  it("handler can render a response", async () => {
    class AppError extends Error { name = "AppError"; }
    class C extends Base {
      async index() { throw new AppError("fail"); }
    }
    ...
    C2.rescueFrom(AppError, () => { rescued = true; });
    const c = new C2();
    await c.dispatch("index", makeRequest(), makeResponse());
    expect(rescued).toBe(true);
  });
  ...
  it("rescue handler is async", async () => {
    class AsyncError extends Error { name = "AsyncError"; }
    class C extends Base {
      async index() { throw new AsyncError("async"); }
    }
    let handled = false;
  ...
  it("most specific rescue handler wins", async () => {
    class BaseError extends Error { name = "BaseError"; }
    class SpecificError extends BaseError { name = "SpecificError"; }
    class C extends Base {
      async index() { throw new SpecificError("specific"); }
    }
    let which = "";
    C.rescueFrom(BaseError, () => { which = "base"; });
    C.rescueFrom(SpecificError, () => { which = "specific"; });
  ...
  it("rescue from standard Error", async () => {
    class C extends Base {
      async index() { throw new Error("generic"); }
    }
    let rescued = false;
    C.rescueFrom(Error, () => { rescued = true; });
