...
describe("ActionController redirecting", () => {
  it("redirect_to with path", async () => {
    class C extends Base { async index() { this.redirectTo("/posts"); } }
    const c = new C();
  ...
  it("redirect_to with full URL", async () => {
    class C extends Base { async index() { this.redirectTo("https://example.com/other"); } }
    const c = new C();
    await c.dispatch("index", makeRequest(), makeResponse());
  ...
  it("redirect_to with 301", async () => {
    class C extends Base { async index() { this.redirectTo("/new", { status: 301 }); } }
    const c = new C();
    await c.dispatch("index", makeRequest(), makeResponse());
  ...
  it("redirect_to with see_other", async () => {
    class C extends Base { async index() { this.redirectTo("/done", { status: "see_other" }); } }
    const c = new C();
    await c.dispatch("index", makeRequest(), makeResponse());
  ...
  it("redirect_to sets HTML body with link", async () => {
    class C extends Base { async index() { this.redirectTo("/target"); } }
    const c = new C();
    await c.dispatch("index", makeRequest(), makeResponse());
  ...
  it("redirect_back uses referer", async () => {
    class C extends Base { async index() { this.redirectBack({ fallbackLocation: "/fallback" }); } }
    const c = new C();
  ...
  it("redirect_back uses fallback when no referer", async () => {
    class C extends Base { async index() { this.redirectBack({ fallbackLocation: "/home" }); } }
    const c = new C();
    await c.dispatch("index", makeRequest(), makeResponse());
  ...
  it("redirect_back with custom status", async () => {
    class C extends Base { async index() { this.redirectBack({ fallbackLocation: "/", status: 303 }); } }
    const c = new C();
    await c.dispatch("index", makeRequest(), makeResponse());
    expect(c.status).toBe(303);
  });
  ...
  it("redirect then redirect throws", async () => {
    class C extends Base { async index() { this.redirectTo("/a"); this.redirectTo("/b"); } }
    const c = new C();
  ...
  it("redirect in before_action prevents action", async () => {
    const log: string[] = [];
    class C extends Base { async index() { this.render({ plain: "ok" }); log.push("action"); } }
