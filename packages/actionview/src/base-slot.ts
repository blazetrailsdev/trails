 ...
 * at `template.ts` module scope — so that edge closes the cycle
...
export let _Base: { annotateRenderedViewWithFilenames: boolean } | undefined;

/** @internal */
export function _setBase(base: { annotateRenderedViewWithFilenames: boolean }): void {
  _Base = base;
}
