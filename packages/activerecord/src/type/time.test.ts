...
describe("TimeTest", () => {
  it("default year is correct", () => {
    const type = new Time();
    const result = type.cast({ 4: 10, 5: 30 }) as Temporal.Instant;
    expect(result).toEqual(
      new Temporal.PlainDateTime(2000, 1, 1, 10, 30, 0).toZonedDateTime("UTC").toInstant(),
    );
  });
  ...
  it("serialize wraps the cast time in Type::Time::Value", () => {
    ...
    expect((serialized as Value).getobj().toString()).toBe("2000-01-01T10:30:00Z");
  ...
  it("serialize_cast_value is equivalent to serialize after cast", () => {
    ...
    const value = type.cast("1999-12-31T12:34:56.789-10:00") as Temporal.Instant;
