 ...
 *
 * **Internal implementation — test files must call {@link useHandlerFixtures}
 * instead.** This lower-level entry point takes an explicit `getAdapter` thunk
 * and runs a per-test delete/reseed. It was once a documented escape hatch for
 * suites that could not use the handler-resolved path (non-transactional suites
 * committing across connections, multi-database suites seeding through
 * model-specific connections, adapter-owning suites). Every one of those call
 * sites has since converged: non-transactional and caller-supplied-connection
 * needs are now expressed through `useHandlerFixtures`' `useTransactionalTests`
 * / `connection` options, which forward the resolved thunk here. The only
 * surviving callers are `useHandlerFixtures` and this module's own tests, so the
 * export stays but is `@internal`.
 *
