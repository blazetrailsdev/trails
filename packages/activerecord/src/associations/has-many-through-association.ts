...
function throughScopeAttributes(assoc: HasManyThroughAssociation): Record<string, unknown> {
  ...
  // build time, otherwise fall back to `self.scope` (the HMT relation).
  // `whereValuesHash(throughTable)` below filters the equality predicates to
  // the through model's table, so target-table predicates carried by either
  // relation are dropped rather than leaking into the join row / delete query.
