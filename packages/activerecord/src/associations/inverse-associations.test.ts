...
describe("AutomaticInverseFindingTests", () => {
  ...
  // Deliberate extras relocated from reflection.test.ts (RFC 0043): these exercise the
  // same automatic-inverse machinery but have no verbatim analog in Rails'
  // inverse_associations_test.rb, so they remain deliberate extras on this file. Their
  // models use bespoke names (registered under unique keys) to avoid clobbering the
  // canonical models shared across this describe.
  ...
  it("has many with scope should not find inverse automatically unless automatic scope inversing", () => {
    // Without automatic_scope_inversing, scoped associations should not find inverse
    {
      class ScopeCompany extends Base {
        static {
          this.attribute("id", "integer");
          this.hasMany("scopeContracts", { scope: (rel: any) => rel, className: "ScopeContract" });
        }
      }
  ...
  it("scoped belongs to on inverse side blocks automatic inverse", () => {
    ...
    class Magazine extends Base {
      ...
      static {
        ...
        this.belongsTo("publisher", { scope: (rel: any) => rel });
      }
    }
