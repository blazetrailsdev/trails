...
describe("HasManyAssociationsTest", () => {
  ...
  // The three `clients_of_firm` delete/build tests below are blocked on the
  // canonical `Client` model: its `validate(async () => await this.firm)`
  // callback runs on trails' synchronous `runValidationsBang` chain during the
  // has_many delete/build path and throws "Async callback on sync chain". This
  // is an impl-level deviation (Rails' nullify-`delete` uses `update_all` and
  // bypasses validations; an async validation can't run on the bang chain),
  // tracked under RFC 0019 follow-up `hm-clients-of-firm-delete-async-validate`.
