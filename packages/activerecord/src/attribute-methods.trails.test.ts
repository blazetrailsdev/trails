...
describe("AttributeMethodsTest (trails)", () => {
  ...
  it("aliasAttribute generates the alias when mass generation already ran", () => {
    // attribute_methods.rb:66-74 — ActiveRecord's `alias_attribute` override.
    // Aliases are lazily generated (:76-78), so once
    // `@alias_attributes_mass_generated` is set an alias declared afterwards is
    // generated by the override's own batch; without it the alias reaches no
    // generator at all.
