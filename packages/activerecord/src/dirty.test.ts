...
//
// Needs a PG IDENTITY column (GENERATED BY DEFAULT AS IDENTITY), created via the
// same raw DDL Rails' postgresql_specific_schema.rb uses, against the pooled
// `Base.connection` (no adapter bypass). Rails' compound `PostgreSQLAdapter &&
// supports_identity_columns?` guard extracts to the feature-only `identity_columns`
// gate; `supports_identity_columns?` is PostgreSQL-only, so the runtime backend
...
describeIfSupports("identity_columns", "DirtyTest", () => {
  ...
  beforeEach(async () => {
    const conn = Base.connection as unknown as {
      ...
      schemaCache?: { clear(): void };
    };
    await conn.execute(`DROP TABLE IF EXISTS cpk_postgresql_identity_table CASCADE`);
    await conn.execute(`
  ...
  afterEach(async () => {
    await (Base.connection as unknown as { execute(sql: string): Promise<unknown> }).execute(
      `DROP TABLE IF EXISTS cpk_postgresql_identity_table CASCADE`,
    );
  });
