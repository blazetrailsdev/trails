...
export class Base extends Model {
  ...
  constructor(attrs: Record<string, unknown> = {}, initBlock?: (record: Base) => void) {
    ...
    if (hasMultiparameterKeys(attrs)) {
      ...
      if (!wasSuppressed) {
        // Rails runs `initialize_internals_callback` up Base's MRO:
        // Inheritance (inheritance.rb:349) then Scoping (scoping.rb:54); Core's
        // (core.rb:851) is empty. base.ts orchestrates that chain explicitly.
  ...
  private _performInsert(attributeNames?: string[]): void {
    ...
    this._pendingOperation = (async () => {
      ...
      const rawReturn = await _Persistence._insertRecord.call(
        ctor,
        adapter as any,
        valuesMap,
        returning,
