...
describe("TaggedLoggingWithoutBlockTest", () => {
  ...
  it("shares tags across threads", () => {
    logger.tagged("shared").info("shared message");
    expect(output.string).toContain("[shared]");
  });
  ...
  it("does not share the same formatter instance of the original logger", () => {
    ...
    const base2 = new Logger(out2);
    const logger2 = taggedLogging(base2);
    logger.tagged("L1").info("msg1");
    logger2.tagged("L2").info("msg2");
    expect(output.string).toContain("[L1]");
    expect(out2.string).toContain("[L2]");
  });

  it("keeps broadcasting functionality", () => {
    logger.info("broadcast test");
    expect(output.string).toContain("broadcast test");
  });

  it("accepts non-String objects", () => {
    logger.tagged(42 as any).info("number tag");
    expect(output.string).toContain("[42]");
  });

  it("keeps formatter singleton class methods", () => {
    // JS doesn't have singleton class methods; verify formatter works
    const fmt = (_s: string, _d: Date, _p: string, msg: string) => `${msg}\n`;
    const base = new Logger(output);
    base.formatter = fmt;
    const tagged = taggedLogging(base);
    tagged.info("formatter test");
