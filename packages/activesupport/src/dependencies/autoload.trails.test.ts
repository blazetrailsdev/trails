...
import { API, ModuleKind } from "typescript/unstable/sync";
...
describe("ActiveSupport::Autoload registry", () => {
  it("has no runtime imports", async () => {
    ...
    const api = new API();
    try {
      const { outputText } = api.transpileModule(source, {
        compilerOptions: { module: ModuleKind.ESNext, target: ts.ScriptTarget.ES2022 },
      });
      const file = api.createSourceFile("autoload.js", outputText);
