 ...
 *
 * `getCryptoAsync` / `getFsAsync` / `getPathAsync` are the exception since RFC
 * 0135 moved crypto-adapter and fs-adapter into `@blazetrails/ruby-compat`: a
 * dynamic import of a Node builtin is a leaf violation there, so those getters
 * now resolve through the same sync bootstrap `getCrypto()` / `getFs()` use and
 * priming buys their seats nothing. They stay in the list so the file keeps
 * registering every adapter, and so a backend that fails to resolve fails here
 * rather than deep in a caller.
 *
