...
describe("RescuableTest", () => {
  it("rescue from with method", () => {
    ...
    rescueFrom.call(target, Error, { with: "handleError" });
  ...
  it("rescue from with block", () => {
    const handled: Error[] = [];
    const target = {};
    rescueFrom.call(target, Error, { with: (e: Error) => handled.push(e) });
  ...
  it("rescue from with block with args", () => {
    const log: string[] = [];
    const target = {};
    rescueFrom.call(target, TypeError, { with: (e: Error) => log.push(e.message) });
  ...
  it("rescues defined later are added at end of the rescue handlers array", () => {
    const log: string[] = [];
    const target = {};
    rescueFrom.call(target, Error, { with: () => log.push("first") });
    rescueFrom.call(target, TypeError, { with: () => log.push("second") });
