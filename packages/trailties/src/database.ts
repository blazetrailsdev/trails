...
export async function connectAdapter(config: DatabaseConfig): Promise<DatabaseAdapter> {
  ...
  switch (adapter) {
    ...
    case "sqlite": {
      ...
      // in the default better-sqlite3 driver. Skip the auto-import when the app
      // has already registered a driver (e.g. via `import ".../sqlite/node-sqlite"`
      // at the top of config/database.ts — generated by `trails new --sqlite-driver`).
      const { getSqlite } = await import("@blazetrails/activerecord/sqlite-adapter");
      let driverPreregistered = false;
      try {
        getSqlite();
      ...
      if (!driverPreregistered) {
        try {
          await import("@blazetrails/activerecord/sqlite/better-sqlite3");
        } catch (cause) {
          throw new Error(
            "trailties needs the `better-sqlite3` package to open a SQLite database. " +
              ...
              "`@blazetrails/activerecord/sqlite-adapter`.",
            { cause },
          );
        }
      }
