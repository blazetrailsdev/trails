...
type ClassMethods = AttributeMethodHost & {
  ...
  attributeMethodAffix(...affixes: Array<{ prefix: string; suffix: string }>): void;
  ...
  defineDirtyAttributeMethods(attrName: string): void;
...
export function aliasAttribute(this: ClassMethods, newName: string, oldName: string): void {
  ...
  this.defineDirtyAttributeMethods(newName);
}
...
export function defineAttributeMethod(
  ...
  CodeGenerator.batch(_owner, __FILE__, __LINE__, (owner) => {
    ...
    this.defineDirtyAttributeMethods(as);
 ...
 */
export function defineDirtyAttributeMethods(this: ClassMethods, attrName: string): void {
  ...
  for (const [methodName, target] of binding) {
    if (this.isInstanceMethodAlreadyImplemented(methodName)) continue;
