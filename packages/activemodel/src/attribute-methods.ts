...
export function defineAttributeMethod(
  ...
  CodeGenerator.batch(_owner, __FILE__, __LINE__, (owner) => {
    ...
    // Rails declares the dirty cascade with `attribute_method_suffix`
    // (dirty.rb), so it is generated by this same path and comes back after an
...
/**
 * Generate per-attribute dirty methods into the class's generated-attribute-methods
 ...
 * `instance_method_already_implemented?` the one guard — a class-body method
 * outranks the module and survives, an inherited GENERATED one does not count
 * as hand-written — and makes `undefine_attribute_methods` clear the cascade.
 */
export function defineDirtyAttributeMethods(this: AttributeMethodHost, attrName: string): void {
  ...
  for (const [methodName, target] of binding) {
    // The same template-method dispatch `define_attribute_method_pattern` uses
    // (attribute_methods.rb:326): ActiveRecord overrides the predicate to reject
    // a name the class itself defines and to raise for one Active Record owns.
    if (
      typeof alreadyImplemented === "function"
