...
describe("FloatTest", () => {
  it("type cast float", () => {
    ...
    expect(type.cast("1")).toEqual(new Number(1.0));
  });

  it("type cast float from invalid string", () => {
    const type = new Types.FloatType();
    assertNil(type.cast(""));
    expect(type.cast("1ignore")).toEqual(new Number(1.0));
    expect(type.cast("bad1")).toEqual(new Number(0.0));
    expect(type.cast("bad")).toEqual(new Number(0.0));
  });
