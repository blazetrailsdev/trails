 ...
 *
 * Schema-drift policy:
 * - Unknown type key: falls back to "value" type + one-time console.warn per key.
 * - v mismatch: throws AttributeSetCoderError.
 * - Attr in envelope but not in schema: kept as additional attribute.
 * - Attr in schema but not in envelope: resolves via defaultAttributes → schema default → Uninitialized.
 *
