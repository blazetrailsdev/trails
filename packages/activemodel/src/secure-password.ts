  ...
  if (validations) {
    ...
    modelClass.validate((record: Model) => {
      ...
      if (challenge != null && challenge !== false) {
        // secure_password.rb:142 — `record.public_send(:"#{attribute}_digest_was")
        // if record.respond_to?(:"#{attribute}_digest_was")`. The reader is
        // generated by `ActiveModel::Dirty`'s `_was` suffix (dirty.rb:243), so a
        // model that does not include the module simply has no `digest_was`.
