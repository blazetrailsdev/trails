...
export default defineConfig(
  ...
  // src and tests are split into two blocks so each area burns down
  // independently. The grandfathered violators live in
  // eslint/no-explicit-any-{src,test}-exclude.json (generated by
  // scripts/generate-no-explicit-any-allowlist.mjs) and are turned `off` by the
  // trailing override block below. Clean files are enforced now and cannot
  // regress; a burndown PR fixes files and drops their entries from the JSON.
  // NEVER hand-add an entry to silence a new violation — fix the `any`.
  {
    files: ["packages/activerecord/src/**/*.ts"],
