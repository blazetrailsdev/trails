 ...
 *
 * `@blazetrails/ruby-compat` only pays for itself if the tree actually CALLS
 * it. Without a gate, the package relocates today's duplicates and the tree
 * grows fresh ones — which is the exact history that produced them:
 * `activesupport/src/core-ext/regexp.ts:18` has been the canonical
 * `regexpEscape` for a long time, and three byte-identical private copies exist
 * anyway (`activerecord/src/support/quote-regex.ts:27`,
 * `activerecord/src/support/run-token.ts:23`,
 * `trailties/src/generators/trails-actions.ts:191`).
 *
 * So: a function or class declared OUTSIDE `packages/ruby-compat/` whose name
 * is a ruby-compat export name, or a registered known-alias of one, is an
 * error. The register lives in the scope module beside this file, seeded from
 * today's tree, each entry carrying the CONTEXT that makes the name
 * unambiguous — Rails-anchored homonyms (`Cache::Store#fetch`, `Session#dig`,
 * `Parameters#dig`, `Core#<=>` as `compare`) are ports and must not be flagged.
 *
 ...
 * (`structural-duplicate-detector-report`, RFC 0129) precisely because its
 * false-positive rate is unknown until it runs against the real tree. Shipping
 * the name-based rule first and gated beats shipping the structural one late
 * and ungated.
 *
 ...
 * converges it onto ruby-compat, and a new row is NEVER added to cover new
 * code. Adding a row for the code in your own diff inverts the whole mechanism
 * — the remedy for a new flag is to call the primitive.
 *
 * Scope-module + only-shrink-exclude-JSON precedent: `no-raw-sql.mjs` /
 * `no-raw-sql-scope.mjs` / `no-standalone-associations-exclude.json`.
 */
