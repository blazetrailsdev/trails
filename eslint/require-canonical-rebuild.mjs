...
/** Guard against a cyclic `const A = [...B]; const B = [...A];`. */
const MAX_NAME_LIST_DEPTH = 8;

/**
 * The canonical table names a `rebuildCanonicalTables()` call proves it
 * restores: the static strings of its second argument's array literal, read
 * through a module-level (or any single-write) `const` binding and through
 * spreads of further such arrays.
 *
 * Returns `null` for a list this pass cannot read — a parameter, a call's
 * return value, a computed element. That is the FAIL-CLOSED direction: the
 * caller then credits the call with restoring nothing, so the file's canonical
 * drops still have to be proven by some other restore. It used to mean "assume
 * it restores everything", which exempted every drop in the file on the
 * strength of one unreadable argument.
 */
function rebuiltTableNames(node, resolve, depth = 0) {
  if (!node || depth > MAX_NAME_LIST_DEPTH) return null;
  if (node.type === "TSAsExpression" || node.type === "TSSatisfiesExpression") {
    return rebuiltTableNames(node.expression, resolve, depth + 1);
  }
  if (node.type === "Identifier") {
    ...
    // A binding written again after its initializer can hold anything by the
    // time the call runs, so it proves nothing.
    if (variable.references.filter((ref) => ref.isWrite()).length > 1) return null;
    return rebuiltTableNames(def.node.init, resolve, depth + 1);
  }
  ...
  for (const element of node.elements) {
    ...
    if (element.type === "SpreadElement") {
      const nested = rebuiltTableNames(element.argument, resolve, depth + 1);
...
const rule = {
  meta: {
    ...
    messages: {
      missingRebuild:
        'Canonical table `{{table}}` is dropped but never restored in this file. A canonical table left dropped drifts the shared per-worker database for every file that runs next. PREFERRED FIX: do not drop the canonical table at all — give this file\'s DDL a bespoke table name, or run it against a private adapter (`:memory:` or a tmpdir file) and add it to the `privateAdapter` group in eslint/require-canonical-rebuild-exclude.json. Reaching for `await rebuildCanonicalTables(adapter, ["{{table}}"])` is the OLD remedy and is now frozen by RFC 0079 (`no-new-rebuild-canonical-tables`): it only works if this file is already in eslint/rebuild-canonical-tables-callers.json with room to spare, and that baseline never grows. Adding a caller here is how three sites appeared after RFC 0079 was written — which is exactly what the ratchet now stops.',
    ...
    return {
      ...
      CallExpression(node) {
        ...
        if (name === "dropTable") {
          ...
          for (const table of rebuiltTableNames(node.arguments[1], resolve) ?? []) {
            rebuilt.add(table);
          }
