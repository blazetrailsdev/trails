...
const rule = {
  ...
  create(context) {
    ...
    function isRowCallbackParam(fn, seen) {
      ...
      // Tested by shape, not by method name: the row set is either the callee's
      // object (`tables.map(cb)`) or another argument (`eachRow(rows, cb)`,
      // `pMap(rows, cb)`, `Array.from(rows, cb)`). A method-name list would
      // miss every non-member spelling.
