...
- `class Module; include Concerning; end` (core_ext/module/concerning.rb:136) credits `Module::Concerning#concern`/`#concerning` to `Module`, whose first file is aliasing.rb. The methods are defined in concerning.rb:114,132 and ported at core-ext/module/concerning.ts, where they are matched; scoped to aliasing.rb so only the duplicate credit is dropped.
  - `concerning`, `concern` (only in: `core_ext/module/aliasing.rb`)
