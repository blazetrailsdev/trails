...
| `active_connections?` / `active_workers?` / `any_changes?` / `default_scopes?` / `eligible_waiters?` / `nested_attributes?` / `saved_changes?` | `is*` / camel / `has*` (the body is an "are there any?" test)           | `active_connections?` → `isActiveConnections` or `activeConnections` or `hasActiveConnections` or `_isActiveConnections` or `_activeConnections` or `_hasActiveConnections` |
...
collection it names (`active_connections?` / `active_workers?` / `any_changes?` / `default_scopes?` / `eligible_waiters?` / `nested_attributes?` / `saved_changes?`); a configuration flag such as
