...
Sized so each PR fits the LOC ceiling and is independently mergeable. Two tracks run in parallel after the bootstrap PRs.
...
| `rbconfig`, `$PROGRAM_NAME`, `ARGV`, `STDOUT`/`STDERR`, `Kernel#system` | platform / process                             | `RUBY_PLATFORM()`, `argv`, `stdout`/`stderr` from `@blazetrails/ruby-compat`, and `getChildProcess()` from `@blazetrails/activesupport/child-process-adapter` |
