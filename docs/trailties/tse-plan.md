...
- [x] ~30 LOC (#2370): render overload fallback gap — shipped in #2390 (single conditional-generic signature).
- [x] ~20 LOC (#2370): collection `locals` type `Omit`s auto-injected keys — shipped in #2407.
- [x] ~50 LOC (#2365): semantic diagnosis tests for render conditional generic — shipped in #2406.
- [x] ~30 LOC (#2365): multi-format intersection type `buildViews` integration test — shipped in #2407.
- [x] ~30 LOC: collection `as` option narrowed to generic literal — shipped in #2412.
...
- [x] ~1 LOC (#2416): `output-safety-helper.ts` `raw()` `OutputBuffer.toString()` coercion bug — replaced with `htmlSafe(buf.toStr())`.
...
- [x] Virtualizer inline `RenderContext` interface (in `tse.ts` `buildPreamble`) sync check — shipped in #2416 (test in `tse.test.ts` asserts member-set match against `TseRenderContext`).
