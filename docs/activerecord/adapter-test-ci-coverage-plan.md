...
| M-3  | **temp-table + index DDL** — **RESOLVED (this PR)**                      | `active-schema.test.ts` `indexes in create`                                                                                      | `visitTableDefinition` included the auto-generated `id` column in the `(…)` clause even when `as:` was set. MySQL `CREATE TABLE … AS SELECT` should emit only inline indexes in the parens, not column definitions. Root: `TableDefinition` constructor added the id column unconditionally regardless of `as:`; Rails' `set_primary_key` guards with `if id && !as` (abstract/schema_definitions.rb:396).                                                                                                      | Fixed in `abstract/schema-definitions.ts`: (1) constructor guards id-column insertion with `!this.as`; (2) `setPrimaryKey` guards with `                                                                                                                                     |     | this.as`. The MySQL `visitTableDefinition`maps`o.columns`unconditionally (matching Rails abstract + MySQL`visit_TableDefinition`, neither of which has an `as:`guard in the visitor). All`ActiveSchemaTest` tests pass. |
