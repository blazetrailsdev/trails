...
fixes have brought that down to **3** (M-1a/M-1b). P-9 resolved in PR D.
Everything else is resolved — this doc tracks only what remains.
...
### PostgreSQL — 0 failed (P-9 resolved in PR D)
...
1. ~~**P-9** — PG schema-dump type shorthand.~~ ✅ Fixed in PR D.
...
4. **Wire the lane in** (~40 LOC) — add the adapter-dir step to `postgres-tests`
   / `mysql-tests` (relocate from PR #2863's prototype jobs). `RUN_ADAPTER_DIRS=1`
   gate added to `vitest.config.ts` in PR D for local verification. The CI step
   (`.github/workflows`) is the remaining work here — recommend non-blocking
   (`continue-on-error`) for PG now that P-9 is green, hold MySQL until M-1 lands.
