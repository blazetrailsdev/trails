...
| **Association collection proxy method_missing delegation + Comment.allAsMethod missing** — `developer.projects.allAsMethod().first()` and `(Post.find(1) as "project").comments.allAsMethod().first()` require the proxy to surface `Project.allAsMethod` / `Comment.allAsMethod`; the latter is unported | `association collection method missing scoping not readonly` (1)                                                                     | `Project.allAsMethod()` and `Project.allAsScope()` exist in `project.ts:62–65`. `Comment.allAsMethod` is absent: `comment.rb:58` has `def self.all_as_method; all; end` but `comment.ts` only has the `allAsScope` named scope. Both the missing static method (~3 LOC) and the collection proxy delegation path (~30 LOC) need fixing. |
