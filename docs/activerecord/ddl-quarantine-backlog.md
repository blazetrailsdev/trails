...
| `persistence.test.ts`                          |      389      | `persistence_test.rb` — ⚠️ 38 pre-existing `expect(true).toBe(true)` stubs (composite-key + `queryConstraints`); remove in rewrite |
...
| File                                        | Tests skipped | Rails counterpart                           |
| ------------------------------------------- | :-----------: | ------------------------------------------- |
| `encryption/contexts.test.ts`               |       9       | `encryption/contexts_test.rb`               |
...
| `validations/length-validation.test.ts`       |       5       | `validations/length_test.rb`          |
...
These files have no bespoke DDL; skipping them would lose coverage for zero benefit.
