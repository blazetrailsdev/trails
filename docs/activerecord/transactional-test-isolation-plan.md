...
**Status:** planning. Prerequisite for running the live-DB `adapters/<db>/**`
...
1. **Harness + opt-in (PG-first).** Add transactional begin/rollback to
   `test-setup-ar.ts` behind an opt-in marker; prove it on a small PG DML set;
   confirm rollback undoes writes (and PG DDL). Build the opt-out mechanism.
