# `pnpm parity:api:build` — stub generation and per-method expected-calls JSDoc
...
`pnpm parity:api:build` closes both gaps: it generates missing method stubs in the
...
From-scratch stub generated by `parity:api:build`:
 ...
 * ported.
 *
 * @missingRailsCall exec_queries — unported (parity:api:build stub)
 * @missingRailsCall scheduled? — unported (parity:api:build stub)
 */
...
comparison covers), `parity:api:build` takes the `missingMethods` list that
...
(`pnpm parity:api:build --package activerecord --file relation.ts`, no bare "build
...
swamp `parity:api`'s matched counts with hollow matches; therefore stubs
...
`parity:api:calls` red. This is handled, not accidental: the stub's
...
`unported (parity:api:build stub)` reason string keeps these rows distinguishable
...
On every run, for every **existing** matched method, `parity:api:build`:
...
1. Computes the current wide missing-call set for the pair — reusing the
   ...
   `parity:api:build` does not re-derive; it reads the artifact and fails if stale
...
3. Diffs:
   - **newly missing** (in artifact, not tagged): add a tag **only when there
     ...
     0044/0047 default strings / `unported (parity:api:build stub)`) gets **no tag**
     (RFC 0083). A placeholder tag justifies nothing — `justifies()` rejects
     it, and it must, since `parity:api:build` would otherwise mint one per
     ...
     `parity:api:build --package <pkg> --dry-run` reports zero files changed on a
   ...
   - **now satisfied** (tagged, not in artifact): drop the tag. If its reason
     ...
     `unported (parity:api:build stub)`), drop silently. If it is human-authored
...
- **Method moved files.** The baseline and tags key on `tsFile`. `parity:api:build`
  ...
  where it currently lives; when the `parity:api:moves` relocation later happens,
...
- **Alias methods / multi-candidate mappings.** Tags store the Ruby call
  ...
  `parity:api` matches aliases today).
...
in the `tasks` repo) and is already honored by `parity:api:extra` as of trails
...
its migration, and nothing here changes `@missingRailsCall` or the `parity:api:build`
design.
...
that known, and why?" — for two different gates (the `parity:api:calls`
missing-call ratchet vs the `parity:api:extra` extra-surface
...
- **Empty-reason handling — a tag with no reason is a hard error, for both
  ...
  - `@noRailsEquivalent`: `noRailsEquivalentReason` runs inside the TS
    extractor, so every `parity:api:extra` / `parity:api` invocation gates it.
  - `@missingRailsCall`: `parseJsdoc`'s sole caller is `parity:api:build`, an opt-in
    developer command with no CI job. `pnpm parity:api:reasons`
  ...
  `pnpm parity:api:detached` (`lint-detached-jsdoc-tags.ts`, same CI job) covers that
...
- **Reader.** `@noRailsEquivalent` is read through `ts.getJSDocTags`
  ...
  comment text (`TAG_LINE`, `build.ts:78`) because `parity:api:build` must **write**
...
- **Where the error is raised.** `@noRailsEquivalent` fails during extraction;
  `@missingRailsCall` fails when `parity:api:build` parses the block it is about to
...
- **`@noRailsEquivalent`** — "this IS public API surface, deliberately, and
  ...
  `parity:api:extra`, reported in the `Allowed` column rather than hidden, with the
...
The `parity:api:extra` report keeps tagged extras **visible** in the `Allowed`
...
`parity:api:extra` prints the count it dropped ("Excluded by kind: N") and reports it
...
demands. Every one passed `parity:api:extra` cleanly for as long as it existed,
because each reason was accurate about its mechanism and merely drew
"therefore permanent" from it. `parity:api:extra` gates the opposite direction only:
...
- The owner is whoever schedules RFC 0080 work; the trigger is checkable from
  the `parity:api:extra` JSON report alone — `tagged.total`, which the stats DB
  already ingests, plus `tagged.classification.convergeable`, which sits in
  the same `tagged` object of that report.
- `tagged.classification.unclassified` is **not** a trigger, even though the
  report still carries it. The gate above fails any run with a non-zero
  unclassified count, so the field is pinned at 0 on `main` and can never
  cross a threshold. `convergeable` replaces it: a CONVERGEABLE tag is a
  placeholder for registered work, so a rising count is exactly the
  re-accumulation this cadence exists to catch, and unlike `unclassified` it
  is deliberately not gated to 0. The lower threshold (5, against 10 for
  `tagged.total`) reflects that every convergeable tag is known debt, whereas
  `tagged.total` also counts permanent tags that need no disposition.
...
3. **JSDoc authoritative; the JSON baselines become generated artifacts**
   derived from the tags (recommended). `parity:api:build --emit-baselines` walks
   ...
   (`parity:api:build --emit-baselines --check`) fails if committed JSON ≠
...
1. Land `parity:api:build` reconcile-only (no stubs), run it on one package
