...
| **C** — sf-only       | `setupFixtures()` only — no transaction, no fixture data, no `fixtures(...)` call (pure handler wiring / shared-DB shield; schema setup orthogonal — 17/31 create no tables, the rest use `createTable`/`defineSchema`) |    31 | Keep `setupFixtures()`; no transactional wrapper wanted (many are PG-DDL suites that break under txn wrapping). No conversion needed beyond confirming intent |
...
associations/association-scope-alias-tracker.trails.test.ts
associations/association-scope.trails.test.ts
associations/cp-count-disable-joins-through.trails.test.ts
associations/disable-joins-association-scope.trails.test.ts
associations/disable-joins-composite-key.trails.test.ts
associations/disable-joins-composite-nested.trails.test.ts
associations/disable-joins-nested-through.trails.test.ts
associations/disable-joins-polymorphic-nonid-pk.trails.test.ts
associations/disable-joins-routing-widening.trails.test.ts
...
associations/getmodelcolumns-virtual-projection.trails.test.ts
...
associations/preloader-bigint-number-key-match.trails.test.ts
callbacks.test.ts
clone.test.ts
