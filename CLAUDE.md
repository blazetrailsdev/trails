...
- A generated reader removed by `undefineAttributeMethods` is not regenerated by
  ...
  (`attribute_methods.rb`), or through construction.

This is a genuine language shortcoming, ratified repo-wide here. Tests mirroring
those Rails arms port the assertions that do not depend on the hook, and there is
no story to proxy records. It does not rule out a `Proxy` on a non-record object
...
(`application/configuration.rb:471-493`, `application/finisher.rb:75-88`).
