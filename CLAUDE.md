...
- **Rails-private helpers must carry `@internal` JSDoc.** The website's
  ...
  Currently scoped to `arel`, `activesupport`, `activemodel`, `actionpack`,
  and `actionview`; widen the `files:` glob in `eslint.config.mjs` when
  rolling out to additional packages.
